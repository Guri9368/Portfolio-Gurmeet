...
const ShaderMaterial = ({
  ...
  useFrame(({ clock }) => {
    ...
    if (t - lastFrame < 1 / maxFps) return;
    lastFrame = t;
  ...
  const getUniforms = () => {
    ...
    for (const key in uniforms) {
      ...
      switch (u.type) {
        case "uniform1f":
          prepared[key] = { value: u.value, type: "1f" };
          break;
        case "uniform3fv":
          prepared[key] = {
          ...
          break;
        case "uniform1fv":
          prepared[key] = { value: u.value, type: "1fv" };
          break;
        default:
          console.error("Invalid uniform:", key);
      }
    }
...
const Shader: React.FC<{
  ...
  React.useEffect(() => {
    setMounted(true); // Only mount Canvas in browser
  }, []);
